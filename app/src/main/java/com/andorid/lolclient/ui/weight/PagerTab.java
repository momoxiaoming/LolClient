package com.andorid.lolclient.ui.weight;
...
import com.andorid.lolclient.util.UIUtils;

public class PagerTab extends ViewGroup {

	private static final float DEFAULT_INDICATORR_HEIGHT = 0;
	private static final float DEFAULT_TEXT_SIZE = 16;
	private static final int TEXT_TYPE_NORMAL = 0;
	private static final int TEXT_TYPE_BOLD = 1;
	...
	private int mIndicatorColor = 0xFFFF3333;// 0xFF0084FF;// 指示器颜色
	...
	private int mTabTextColorResId = R.color.test; // tab文字颜色   ...可以是color中xml
	private int mTabCount;// tab的个数
	private int mTextType;// 文字类型
	...
	private void init(Context context, AttributeSet attrs, int defStyle) {
		TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.PagerTab);
		mIndicatorHeight = a.getDimensionPixelSize(R.styleable.PagerTab_indicatorHeight,
				(int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP,
						DEFAULT_INDICATORR_HEIGHT, getResources().getDisplayMetrics()));// 默认为10dp
		mTabTextSize = a.getDimensionPixelSize(R.styleable.PagerTab_textSize, (int) TypedValue
				.applyDimension(TypedValue.COMPLEX_UNIT_DIP, DEFAULT_TEXT_SIZE, getResources()
						.getDisplayMetrics()));// 默认为10dp
		mTextType = a.getInt(R.styleable.PagerTab_textType, TEXT_TYPE_NORMAL);// 默认为Circle

		a.recycle();

		// 把一个值从dip转换成px
		mIndicatorHeight = UIUtils.dip2px(context,mIndicatorHeight);
		mDividerPadding = UIUtils.dip2px(context,mDividerPadding);
		mTabPadding = UIUtils.dip2px(context,mTabPadding);
		mDividerWidth = UIUtils.dip2px(context,mDividerWidth);
		mTabTextSize = UIUtils.dip2px(context,mTabTextSize);
		// 创建一个scroller
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		for (int i = 0; i < mTabCount; i++) {
			...
			if (childLayoutParams.height == LayoutParams.MATCH_PARENT) {
				childHeightMeasureSpec = MeasureSpec.makeMeasureSpec(heightSize,
						MeasureSpec.EXACTLY);
		...
		if (totalWidth <= widthSize) {// 如果子Tab的总宽度小于PagerTab，则采用平分模式
			...
			for (int i = 0; i < mTabCount; i++) {
				...
				int childWidthMeasureSpec = MeasureSpec.makeMeasureSpec(splitWidth,
						MeasureSpec.EXACTLY);
	...
	private void onUp(float velocity) {
		if (mMaxScrollX <= 0) {
			...
			mScroller.fling(getScrollX(), 0, -(int) (velocity + 0.5), 0, 0, mMaxScrollX, 0, 0, 270,
					0);
			ViewCompat.postInvalidateOnAnimation(this);
		}
	}
